...
class _ExistingState extends State<ExistingWorkouts> {
  ...
  filterOnDifficculty() {
    ...
    for (WorkoutSession session in sessions) {
      if (filters.contains(session.difficulty)) tempList.add(session);
    }
  ...
  _getSessions() async {
    ...
    List<String> likedRef =
        (await DatabaseService(uid: _user.uid).getLikedRef());
